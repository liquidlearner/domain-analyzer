...
//  analyses your configuration, and generates a self-contained JSON file
//  containing:
//    • Resource inventory — counts and detail for every major PagerDuty object
//      (services, users, schedules, escalation policies, teams, and more)
//    • Shadow stack signals — custom integrations, webhooks, workflows, and
//      automation that will need to be considered alongside core resources
//    • Account metadata — plan, licenses, feature flags, and priorities
//
//  OUTPUT
//  ------
...
//    • Account license info (plan name, seat count) — GET /licenses
//    • Incident priority definitions (P1–P5) — GET /priorities
//    • Services (name, integrations, alert grouping) — GET /services
//    • Alert grouping settings (intelligent grouping config) — GET /alert_grouping_settings
//    • Teams (name, members) — GET /teams
//    • Schedules (layers, users, teams) — GET /schedules
//    • Escalation policies (with team ownership) — GET /escalation_policies
...
//    • Business service → technical service mapping — GET /service_dependencies/business_services/{id}
...
//    • Legacy global event rules — GET /event_rules
...
//  USAGE
//  -----
//    node pd-analyzer-v2.js
//    node pd-analyzer-v2.js --days=90 --output=my-analysis.json
//    node pd-analyzer-v2.js --help
//
//  FLAGS
//  -----
...
EXAMPLE
  node pd-analyzer-v2.js --days=90
  node pd-analyzer-v2.js --rate-limit=150   # extra conservative for large enterprise
...
function printBanner() {
  ...
  log(`${C.bold}  PagerDuty Configuration Analyzer v2${C.reset}`);
...
class PagerDutyClient {
  ...
  constructor({ token, subdomain, rateLimit = 300 }) {
    ...
    // Concurrency gate — prevents thundering herd when Phase 1 fires all fetches
    // simultaneously. Phase 1 launches 23 concurrent calls; without a gate, all 23
    // fire before any response returns (before _reqCount can accumulate), so _throttle()
    // never intervenes. PD sees a burst of 23 requests and rate-limits some of them.
    // listUsers() silently catches the exhausted retries and returns [] — the bug.
    //
    // At 8 max in-flight, Phase 1's first 8 calls fire immediately (listUsers() is
    // #5 in order, so it always gets a slot). The remaining 15 queue and fire as slots
    // open. Burst stays below PD's undocumented short-window limit. Phase 1 still
    // runs in parallel — just with a polite ceiling instead of all-at-once.
    this._maxInFlight  = 8;
    this._inFlight     = 0;
    this._waitQueue    = []; // resolve callbacks for requests waiting on the gate
  }
  ...
  /**
   * Account license info — returns plan name and current licensed seat count.
   * The `current_value` field on each license is the number of active seats.
   * Useful for license comparison in the migration proposal.
   */
  async getLicenses() {
    try {
      const res = await this._request('GET', '/licenses');
      return res.licenses || [];
    } catch { return []; }
  }

  /**
   * Incident priority definitions (P1–P5 or custom).
   * Shows how the customer has named and coloured their severity levels.
   * Only the order and names are captured — no incident content.
   */
  async listPriorities() {
    try {
      const res = await this._request('GET', '/priorities');
      return res.priorities || [];
    } catch { return []; }
  }

  /**
   * Tags defined on the account (service tags, environment tags, etc.).
   * Tags are used for service classification and wave planning.
   */
  async listTags() {
    try { return await this._all('/tags', 'tags'); } catch { return []; }
  }

  /**
   * Legacy global event rules (deprecated, but still active on many older accounts).
   * These predate Event Orchestrations and must be migrated to incident.io alert routes.
   */
  async listEventRules() {
    try {
      const res = await this._request('GET', '/event_rules');
      return res.rules || [];
    } catch { return []; }
  }

  /**
   * Intelligent alert grouping settings — per-service config for AIOps grouping.
   * Provides richer detail than the service-level alert_grouping_parameters field:
   * includes time_window and iag_fields (which alert fields drive grouping).
   * Capped at 500 to avoid excessive calls on very large accounts.
   */
  async listAlertGroupingSettings() {
    try { return await this._all('/alert_grouping_settings', 'alert_grouping_settings', {}, undefined, 500); } catch { return []; }
  }
  ...
  async _request(method, path, params = {}, body = null) {
    // Concurrency gate — hold requests beyond _maxInFlight until a slot opens.
    // This prevents Phase 1's parallel Promise.all() from firing 20+ simultaneous
    // paginating calls, which would all slip through _throttle() before the counter
    // has accumulated enough to trigger the per-minute window check.
    ...
    try {
      ...
      this._inFlight--;
      // Wake the next queued request (FIFO), if any
      if (this._waitQueue.length > 0) this._waitQueue.shift()();
    }
  }
...
async function fetchConfig(client, days, onProgress) {
  ...
  const [
    ...
    incidentWorkflows,
    eventOrchestrations,
    [automationActions, automationRunners],
    responsePlays,
    maintenanceWindows,
    slackConnections,
    [statusPages, statusDashboards],
    abilities,
    rulesets,
    priorities,
    tags,
    ...
    client.listIncidentWorkflows(),
    client.listEventOrchestrations(),
    Promise.all([client.listAutomationActions(), client.listAutomationRunners()]),
    client.listResponsePlays(),
    client.listMaintenanceWindows(),
    client.getSlackConnections(),
    Promise.all([client.listStatusPages(), client.listStatusDashboards()]),
    client.getAbilities(),
    client.listRulesets(),
  ...
  // Per-service rule sampling, team member enrichment, business service dep
  // mapping, and stale detection can all run simultaneously.
  report('Phase 2: Per-service enrichment and stale detection...');
  ...
  const [
    ...
    client.enrichTeamsWithMembers(rawTeams),
    rawBusinessServices.length
      ? client.enrichBusinessServicesWithDeps(rawBusinessServices)
      : Promise.resolve([]),
    client.listServiceEventRules(services),
    client.listRecentIncidentServiceIds(services.map(s => s.id), days),
  ]);

  report(`Phase 2 complete — ${activeServiceIds.size} active / ${services.length - activeServiceIds.size} stale`);
  ...
  return {
    ...
    // New in v2:
    priorities, tags, eventRules, licenses, alertGroupingSettings,
  };
}
...
function buildReport({ domain, config, days }) {
  ...
  return {
    ...
    account: {
      ...
      // License info — plan name and total licensed seat count.
      // current_value is the number of active paid seats on this account.
      licenses: (config.licenses || []).map(l => ({
        name:          l.name,
        current_value: l.current_value,  // seat count
        valid_roles:   l.valid_roles || [],
      })),
      // Priority definitions — P1–P5 names and colours.
      // Useful for understanding how the customer classifies incident severity.
      priorities: (config.priorities || []).map((p, i) => ({
        order:       i + 1,
        name:        p.name,
        color:       p.color,
    ...
    // Legacy global event rules (deprecated endpoint, but still active on older accounts).
    // These predate Event Orchestrations and must be migrated to incident.io alert routes.
    legacy_event_rules: {
      total: (config.eventRules || []).length,
    },
    // Tags defined on the account (environment tags, tier tags, team ownership tags, etc.).
    // Useful for wave planning — teams can be grouped by environment tag (prod vs non-prod).
    tags: {
      total: (config.tags || []).length,
      items: (config.tags || []).map(t => ({ name: t.label, id: t.id })),
    },
    // Intelligent alert grouping settings — richer than the per-service flag.
    // Shows time_window and iag_fields (which alert fields drive the grouping model).
    // A high count here means the account is actively using AIOps — call it out
    // as a migration discussion point (incident.io has deduplication, not the same ML model).
    alert_grouping_settings: {
      total: (config.alertGroupingSettings || []).length,
      by_type: (() => {
        const counts = {};
        for (const s of (config.alertGroupingSettings || [])) {
          counts[s.type || 'unknown'] = (counts[s.type || 'unknown'] || 0) + 1;
        }
        return counts;
      })(),
    },
...
const ENDPOINT_PURPOSES = {
  ...
  '/priorities': {
    returns: 'Incident priority definitions (P1–P5 or custom severity levels).',
    fields_used: 'Priority name, colour, order.',
    why: 'Priority definitions show how the customer classifies incident severity. Useful for configuring incident.io severity levels to match existing conventions and for understanding what drives ServiceNow ticket creation (typically P1/P2 only).',
  },
  '/licenses': {
    returns: 'Account license information — plan name and active seat count.',
    fields_used: 'License name, current_value (seat count), valid_roles.',
    why: 'Licensed seat count is used for incident.io license sizing and to frame the cost comparison in the migration proposal. Plan name indicates which PagerDuty tier the customer is on.',
  },
  '/tags': {
    returns: 'Tags defined on the account (environment, tier, team ownership labels).',
    fields_used: 'Tag label, ID.',
    why: 'Tags reveal how the customer organises their service catalog (e.g., env=prod, tier=critical). Used for migration wave planning — teams can be grouped by environment tag.',
  },
  '/event_rules': {
    returns: 'Legacy global event rules (deprecated endpoint).',
    fields_used: 'Rule count only.',
    why: 'Some older PagerDuty accounts still have active global event rules on the deprecated /event_rules endpoint alongside modern Event Orchestrations. These must be migrated to incident.io alert routes.',
  },
  '/alert_grouping_settings': {
    returns: 'Intelligent alert grouping configuration per service.',
    fields_used: 'Type, time_window, iag_fields.',
    why: 'Provides richer alert grouping detail than the service-level field. A high count of intelligent grouping settings indicates the account is actively using PagerDuty AIOps — this is a migration discussion point, as incident.io uses deduplication rather than the same ML grouping model.',
  },
  '/incidents': {
